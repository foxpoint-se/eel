...
`make install` runs pip (via `setup.py`), rosdep (apt packages from `package.xml`), and `pi_ina226` from a pinned git commit (battery hardware). After install, `source source_me.sh` in every new shell before `make build`, `make test`, or `ros2 run`. Later, `make setup` (still after sourcing) re-runs install + build.

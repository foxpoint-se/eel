...
class NavigationActionServer(Node):
    ...
    def handle_accepted_callback(self, goal_handle):
        with self._goal_queue_lock:
            if self._current_goal is not None:
                ...
                self.logger.info(f"New goal put in queue, queue length: {len(self._goal_queue)}")
            else:
    ...
    def goal_callback(self, goal_request):
        position_aquired = all([self.current_position.get("lat"), self.current_position.get("lat")])
    ...
    def execute_callback(self, goal_handle):
        try:
            ...
            self.logger.info(f"Executing new goal, target set to {self.target_position}, distance to target {self.distance_to_target}")
            ...
            with self._goal_queue_lock:
                try:
                    ...
                    self.logger.info(f"Next goal pulled from the queue, queue length: {len(self._goal_queue)}")
                    self._current_goal.execute()
                except IndexError:
                    # No goal in queue
                    self._current_goal = None

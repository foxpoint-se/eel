...
from .eel_depth_controllers import (
    # depth_controller_1,
    depth_controller_for_simulation_1,
)
from .base import DepthConfig, EelDepthState
import rclpy
...
def is_within_accepted_target_boundaries(
    current_level, target_level, tolerance
):
...
class DepthControlNode(Node):
    def __init__(self):
        ...
        # TODO: conditionally set pid settings, based on what we find when
        # tuning hardware
        ...
        self.pid_publisher_base = self.create_publisher(
            Float32, "pid_error_target", 10
        )
        ...
        self.front_tank_pub = self.create_publisher(
        ...
        self.depth_controller = depth_controller_for_simulation_1

    def handle_cmd_msg(self, msg):
        depth_target = msg.depth_target
        pitch_target = msg.pitch_target
        self.depth_controller.set_new_target(
            DepthConfig(depth=depth_target, pitch=pitch_target)
        )
    ...
    def loop(self):
        ...
        next = self.depth_controller.update(state)

        # TODO: do we need to shut them off? or does it take care of itself
        # somehow?
        if next:
            front_msg = Float32()
            front_msg.data = next.front
            rear_msg = Float32()
            rear_msg.data = next.rear
    ...
    def go_to_surface(self):
        raise RuntimeError("go to surface not implemented")
    ...
    def handle_cmd_msg_OLD(self, msg):
        ...
        # TODO: Think about how to handle fluctuating sensor values. The noise
        # from them will result in constant updates from the PIDs.
        # Maybe kill the PIDs when inside an accepted target range and
        # re-initialize them when the vehicle has drifted away.
        # We could also make the simulators return fluctuating values, so that
        # we can code the logic even in simulation mode.
    ...
    # def loop_WORKING_OLD_BUT_MODIFIED(self):
    ...
    #         # merge:a här liksom
    #         # pid_merger = PidMerger()

    #         self.publish_debug_values(
    #             depth_controller_output, pitch_front_tank
    #         )
    ...
    def loop_WORKING_OLD(self):
        if self.pitch_pid_controller and self.depth_pid_controller:
            # if self.last_pitch_controller_output and abs(self.pitch_target -
            # self.current_pitch) <= 1.5:
            ...
            # if self.last_depth_controller_output and abs(self.depth_target -
            # self.current_depth) <= 0.025:
            ...
            # TODO add to status: status --> going to target, target reached
            # etc

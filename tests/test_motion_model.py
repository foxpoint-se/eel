...
def test__when_motor_on_heading_zero__should_advance_along_north() -> None:
    model = MotionModel()
...
def test__when_motor_on_heading_ninety__should_advance_along_east() -> None:
    model = MotionModel()
    model.set_speed_response_rate_per_s(0.0)
    model.set_motor_cmd(1.0)
    model.set_heading_deg(90.0)
...
def test__when_motor_reverse_heading_zero__should_move_along_negative_north() -> None:
    model = MotionModel()
    model.set_speed_response_rate_per_s(0.0)
    model.set_motor_cmd(-1.0)
    model.set_heading_deg(0.0)
    x, y = model.step(1.0)
    assert isclose(x, 0.0, abs_tol=1e-9)
    assert isclose(y, -DEFAULT_REVERSE_CRUISE_MPS)
...
def test__when_exponent_is_two_and_half_throttle__should_reduce_travel() -> None:
    ...
    x, y = model.step(1.0)
    assert isclose(x, 0.0, abs_tol=1e-9)
    assert isclose(y, DEFAULT_FORWARD_CRUISE_MPS * 0.25)
...
def test__when_pitch_increases_while_coasting__should_reduce_horizontal_travel() -> None:
    ...
    model_flat.step(1.0)
    y_before_flat = model_flat.y_m
    model_flat.set_motor_cmd(0.0)
    model_flat.step(0.1)
    dy_flat = model_flat.y_m - y_before_flat

    assert dy_pitched < dy_flat
    assert isclose(dy_pitched, dy_flat * cos(radians(45.0)))
